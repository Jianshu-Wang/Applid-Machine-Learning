...
for current_depth in depth:
    for current_tree in tree:
        ...
        regr_stretch = RandomForestClassifier(n_estimators=current_tree, max_depth=current_depth)
        regr_stretch.fit(data_list_stretched, label_list)
        test_stretch = regr_stretch.predict(test_data_list_stretched)
        acc_stretch = evaluateClassifier(test_stretch, test_label_list)
        print 'Accuracy for decision tree' + str(current_tree) + 'depth' + str(
            current_depth) + '& stretched bounding box is: '
        print acc_stretch

        from sklearn.ensemble import RandomForestClassifier

        regr_bounding = RandomForestClassifier(n_estimators=current_tree, max_depth=current_depth)
        regr_bounding.fit(data_list_bounding, label_list)
        test_bounding = regr_bounding.predict(test_data_list_bounding)
        acc_bounding = evaluateClassifier(test_bounding, test_label_list)
        print 'Accuracy for decision tree' + str(current_tree) + 'depth' + str(current_depth) + '& untouched image is: '
        print acc_bounding
