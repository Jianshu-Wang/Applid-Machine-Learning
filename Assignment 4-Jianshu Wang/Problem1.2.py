...
X = np.array([[3.3, 0.9, 27.6, 0.9, 8.2, 19.1, 6.2, 26.6, 7.2],
...
for cluster_number in range(1, 21):
    ...
    for data_item in range(0, n):
        cluster_label = kmeans.labels_[data_item]
        cluster_center = kmeans.cluster_centers_[cluster_label]
        dis = distance(X[data_item], cluster_center)
        total_Cost += dis
...
plt.plot(cluster_list,cost_list,'o-')
plt.ylabel('Within Groups Sum of Squares')
plt.xlabel('Number of Clusters')
plt.title('Cost vs Cluster Number')
plt.show()